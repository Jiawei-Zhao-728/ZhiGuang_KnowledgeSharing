...
public class SecurityConfig {
    ...
    public SecurityFilterChain securityFilterChain(
            HttpSecurity http,
            Converter<Jwt, ? extends AbstractAuthenticationToken> accessTokenAuthenticationConverter
    ) throws Exception {
        http
    ...
    /**
     * Restrict bearer authentication to access tokens. Refresh tokens are also valid JWTs,
     * but they must only be accepted by the refresh endpoint's explicit body validation path.
     */
    @Bean
        ...
        return jwt -> {
            if (!"access".equals(jwt.getClaimAsString("token_type"))) {
                throw new OAuth2AuthenticationException(
                        new OAuth2Error("invalid_token", "Only access tokens can authenticate API requests", null)
                );
            }
