...
public class SecurityConfig {
    ...
    /**
     * Resource-server bearer auth must only accept short-lived access JWTs.
     * Refresh JWTs are validated separately by AuthService where revocation is checked.
     */
    @Bean
    public Converter<Jwt, AbstractAuthenticationToken> accessTokenJwtAuthenticationConverter() {
        JwtAuthenticationConverter delegate = new JwtAuthenticationConverter();
        return jwt -> {
            if (!"access".equals(jwt.getClaimAsString("token_type"))) {
                throw new InvalidBearerTokenException("Bearer token must be an access token");
            }
